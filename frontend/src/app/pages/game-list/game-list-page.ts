...
export class GameListPage {
    ...
    getfilteredGames(): Game[] {
        return this.games().filter((game) => {
            if (
                this.filters.title &&
                !game.title.toLowerCase().includes(this.filters.title.toLowerCase())
            ) {
                return false;
            }

            const selectedTags = Object.keys(this.filters.tags).filter(
                (tag) => this.filters.tags[tag],
            );
            ...
            if (
                this.filters.releaseFrom &&
                new Date(game.releaseDate) < new Date(this.filters.releaseFrom)
            ) {
                return false;
            }
            if (
                this.filters.releaseTo &&
                new Date(game.releaseDate) > new Date(this.filters.releaseTo)
            ) {
                return false;
            }
